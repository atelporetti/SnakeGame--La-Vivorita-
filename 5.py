...
class main(Tkinter.Tk):
    ...
    def creating_score_board(self):
        self.scoreboard = Tkinter.Label(self, text=f"Score : {self.score}")
    ...
    def creating_snake_head(self):
        self.snake = self.board.create_rectangle(
            1, 1, 11, 11, fill=SNAKE_HEAD_COLOR)
        return
    # Creating Ground

    def creating_playground(self):
        self.board = Tkinter.Canvas(
            self, width=PLAYGROUND_WIDTH, height=PLAYGROUND_HEIGHT, background=PLAYGROUND_COLOR)
    ...
    def game_loss(self):
        self.board.create_text(PLAYGROUND_WIDTH/2, PLAYGROUND_HEIGHT/2,
    ...
    def food_of_snake(self):
        if self.snake_target == None:
            ...
            self.snake_target = self.board.create_oval(
                x1, y1, x1+10, y1+10, fill='yellow', tag="food")
    ...
    def update_snake_body_structure(self):
        ...
        if len(self.roadmap) >= self.bodylength:
            ...
            self.board.create_line(tuple(self.roadmap),
                                   tag="body", width=10, fill=SNAKE_HEAD_COLOR)
        return
